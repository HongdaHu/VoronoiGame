...
struct Halfedge * VoronoiDiagramGenerator::ELleftbnd(struct vPoint *p)
{
	int i, bucket;
	struct Halfedge *he;
...
struct Site * VoronoiDiagramGenerator::intersect(struct Halfedge *el1, struct Halfedge *el2, struct vPoint *p)
{
	struct	Edge *e1,*e2, *e;
	struct  Halfedge *el;
	float d, xint, yint;
...
int VoronoiDiagramGenerator::right_of(struct Halfedge *el,struct vPoint *p)
{
	struct Edge *e;
	struct Site *topsite;
...
struct vPoint VoronoiDiagramGenerator::PQ_min()
{
...
void VoronoiDiagramGenerator::pushGraphEdge(float x1, float y1, float x2, float y2 , int s1n, int s2n)
{
...
void VoronoiDiagramGenerator::line(float x1, float y1, float x2, float y2 , int s1n, int s2n)
{	
	pushGraphEdge(x1,y1,x2,y2,s1n,s2n);
...
void VoronoiDiagramGenerator::clip_line(struct Edge *e)
{
    ...
    if(e!=(struct Edge *)NULL){
        if(e -> reg[0]!=(struct Site *)NULL) s1n = e -> reg[0] ->sitenbr;
        if(e -> reg[1]!=(struct Site *)NULL) s2n = e -> reg[1] ->sitenbr;
    }
//    printf("line(%f,%f,%f,%f)'s sites: %d, %d\n",x1,y1,x2,y2,s1n,s2n);
	line(x1,y1,x2,y2,s1n,s2n);
}
...
{
	struct vPoint *s1 = (vPoint*)p1, *s2=(vPoint*)p2;
