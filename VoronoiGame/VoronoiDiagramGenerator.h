...
private:
	...
	struct Site *intersect(struct Halfedge *el1, struct Halfedge *el2, struct vPoint *p=0);
	...
	void pushGraphEdge(float x1, float y1, float x2, float y2, int s1n, int s2n);

	void openpl();
	void line(float x1, float y1, float x2, float y2 , int s1n, int s2n);
